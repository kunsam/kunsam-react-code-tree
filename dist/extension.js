...
class RoutersCommand {
    ...
    init(context) {
        ...
        context.subscriptions.push(vscode.commands.registerCommand("extension.goFileParentFile", (uri) => {
            ...
            if (!result.parents) {
                ...
                extensionUtil_1.pickFiles2Open(result.parents.map((r) => ({ label: path.relative(config_1.ROOT_PATH, r), target: r })));
            }
        }));
        context.subscriptions.push(vscode.commands.registerCommand("kReactRouterTree.showFileParentsInPick", () => {
            ...
            extensionUtil_1.pickFiles2Open(result.map(r => ({
                label: `${new Array(r.depth).fill('    ').join('')}|-${path.relative(config_1.ROOT_PATH, r.path)}`,
                target: r.path
            })), false);
        }));
        ...
        context.subscriptions.push(vscode.commands.registerCommand("extension.goFileTopParentFile", (uri) => {
            ...
            extensionUtil_1.pickFiles2Open(parents.map((r) => ({ label: path.relative(config_1.ROOT_PATH, r), target: r })));
        }));
        // 右键菜单 getFileAppUrl
    ...
    recursiveGetParents(path, result, preventLoopMap) {
        ...
        const parents = (this.kRouterTree.getFileParents(path).parents || []).filter((a) => a !== path);
    ...
    recursiveGetParentsDepthFirt(path, result, preventLoopMap, depth) {
        if (preventLoopMap.has(path)) {
            return result;
        }
        preventLoopMap.set(path, true);
...
const extensionUtil_1 = __webpack_require__(/*! ../../extensionUtil */ "./src/extensionUtil.ts");
// 文件 - 父节点表
const FILE_PARENTS_MAP = new Map();
// 文件 - 引入表
        ...
        if (mapImports) {
            ...
            try {
                ...
                if (absPath === '/Users/kunsam/Downloads/le-project/wechat-web/src/app/containers/product/detail_buttons/index.jsx') {
                    console.log(id, relativeImports, extensionUtil_1.getFileAbsolutePath(absPath, false), 'relativeImports \n');
                }
 ...
 * @param {string[]} files
 */
function pickFiles2Open(files, isOpenFirst = true) {
